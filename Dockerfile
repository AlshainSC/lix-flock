# Multi-stage Dockerfile for Lix-Flock drone swarm simulation
...
# Pre-configure esbuild version to avoid download during build
ENV ESBUILD_VERSION="0.17.11"

# Compile assets
COPY apps/web_interface/assets apps/web_interface/assets
COPY apps/web_interface/priv apps/web_interface/priv
WORKDIR /app/apps/web_interface
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/lix_flock ./
