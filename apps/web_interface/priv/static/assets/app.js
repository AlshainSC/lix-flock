  ...
  var DroneVisualization = class {
    ...
    drawDrone(drone) {
      if (!drone.position)
        return;
      const { width, height } = this.canvas;
      const x = drone.position.x / 1e3 * width / 2 + width / 2;
      const y = drone.position.y / 1e3 * height / 2 + height / 2;
      ...
      for (let i = 0; i < this.drones.length; i++) {
        for (let j = i + 1; j < this.drones.length; j++) {
          ...
          const distance = Math.sqrt(
            Math.pow(drone1.position.x - drone2.position.x, 2) + Math.pow(drone1.position.y - drone2.position.y, 2) + Math.pow(drone1.position.z - drone2.position.z, 2)
          );
          if (distance <= connectionDistance) {
            const { width, height } = this.canvas;
            const x1 = drone1.position.x / 1e3 * width / 2 + width / 2;
            const y1 = drone1.position.y / 1e3 * height / 2 + height / 2;
            const x2 = drone2.position.x / 1e3 * width / 2 + width / 2;
            const y2 = drone2.position.y / 1e3 * height / 2 + height / 2;
            this.ctx.beginPath();
